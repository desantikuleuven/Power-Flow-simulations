...
- my_build_pf_DGs contains the load as a variable, defined in variable_load(). The load is reduced by a certain flexibility % 
...
- Generators are considered as PQ, hence it was added a constraint for each generator on the P and Q produced, leaving the node voltage as a variable
REMARKS:
...
function my_build_pf_DGs(pm::AbstractPowerModel)
