...
- my_build_ref contains the load as a variable, defined in variable_load(). The load is reduced by a certain flexibility % 
  (present in net_data["load"]["flex_%"] added by function add_load_flexibility).
  The objective function tries to minimize the use of flexibility to solve congestion and voltage problems, where flexibility is defined as 
  the deviation in the programmed load vs the actual load. 
    
  It was also added a congestion constraint, in order to highlight the use of flexibility, where the rate_a is moltiplied
  by a coefficient to reduced every time the cable capacity available. The same effect can be achieved by increasing the 
  load condition to cause a natural violation of the constraint. 

- comp_start_value --> gets as input a dict, key and default value set to 0.0. This funtion simply does
a get(dict,key,default_value). So it returns the value dict[:key]. If such key is not present, it returns the defauylt value. 

- sol_component_value --> something strange.

REMARKS:

- flexibility is introduced in the variable_load constraints.
- Keep an eye on the coefficient for the thermal constraint. 

=#

function my_build_pf(pm::AbstractPowerModel)
