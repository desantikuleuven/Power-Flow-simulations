...
- build_pf_dg_curtail takes into account the possibility to have downards and upwards flexibility and DG curtailment. 
Working principle same as in My_ref_lazy_flex. 
...
- DGs as variables. Defined as PQ buses, their P and Q injection can vary accordingly to the need. Changed the obj function.  

REMARKS:
!!!! Here voltage is unbounded !!!!
=#

function build_pf_dg_curtail(pm::AbstractPowerModel)
    
    variable_bus_voltage(pm, bounded = false)
    variable_gen_power_me(pm, bounded = true)
...
function objective_min_load_gen_variations(pm::AbstractPowerModel; nw = nw_id_default, kwargs...)
    ...
    return JuMP.@objective(pm.model, Min, sum(gen["pg"]-var(pm,nw, :pg, i) for (i,gen) in ref(pm, nw, :gen) if i!=1) +
    sum(gen["qg"]-var(pm,nw, :qg, i) for (i,gen) in ref(pm, nw, :gen) if i!=1) +
    sum(var(pm,nw, :x_p, i) + var(pm,nw, :y_p, i) for i in ids(pm, :load)) + 
    sum(var(pm,nw, :x_q, i) + var(pm,nw, :y_q, i) for i in ids(pm, :load)))
...
function variable_gen_power_real_me(pm::AbstractPowerModel; nw::Int=nw_id_default, bounded::Bool=true, report::Bool=true)
    pg = var(pm, nw)[:pg] = JuMP.@variable(pm.model,
        ...
        start = comp_start_value(ref(pm, nw, :gen, i), "pmax")
    )
...
function variable_gen_power_imaginary_me(pm::AbstractPowerModel; nw::Int=nw_id_default, bounded::Bool=true, report::Bool=true)
    qg = var(pm, nw)[:qg] = JuMP.@variable(pm.model,
        ...
        start = comp_start_value(ref(pm, nw, :gen, i), "qmax")
    )
